	...
	}
	{
		...
		for _, q := range quarter {
			...
			if q.Quarter == 1 {
				...
				if firstDay.Weekday != 3 {
					t.Fatal(fmt.Sprintf("%d年第%d季度的首日星期几有误", year, q.Quarter), firstDay.Weekday, "!=", 3)
					return
				}
				if firstDay.QuarterWeeks != 1 {
					t.Fatal(fmt.Sprintf("%d年第%d季度的首日是季度第几周有误", year, q.Quarter), firstDay.QuarterWeeks, "!=", 1)
			...
			if q.Quarter == 4 {
				...
				if lastDay.Weekday != 3 {
					t.Fatal(fmt.Sprintf("%d年第%d季度的尾日星期几有误", year, q.Quarter), lastDay.Weekday, "!=", 3)
					return
				}
				if lastDay.QuarterWeeks != 14 {
					t.Fatal(fmt.Sprintf("%d年第%d季度的尾日是季度第几周有误", year, q.Quarter), lastDay.QuarterWeeks, "!=", 13)
		...
		for _, m := range month {
			...
			if m.Month == 2 {
				...
				if d := m.Days[len(m.Days)-1].Day; d != 28 {
					t.Fatal(fmt.Sprintf("%d年%d月的天数有误", year, m.Month), d, "!=", 28)
					return
				}
				if w := m.Days[0].Weekday; w != 6 {
					t.Fatal(fmt.Sprintf("%d年%d月1号的星期几有误", year, m.Month), w, "!=", 6)
			...
			if m.Month == 3 {
				...
				if w := m.Days[len(m.Days)-1].Weekday; w != 1 {
					t.Fatal(fmt.Sprintf("%d年%d月31号的星期几有误", year, m.Month), w, "!=", 1)
					return
				}
			}
			if m.Month == 4 {
				...
				if w := m.Days[len(m.Days)-1].Weekday; w != 3 {
					t.Fatal(fmt.Sprintf("%d年%d月30号的星期几有误", year, m.Month), w, "!=", 3)
					return
				}
			}
			if m.Month == 5 {
		...
		if w := firstWeekFirstDay.Weekday; w != 3 {
			t.Fatal(fmt.Sprintf("%d年第一周第一天星期几有误", year), w, "!=", 3)
			return
		}
		firstWeekLastDay := firstWeek.Days[len(firstWeek.Days)-1]
		if m := firstWeekLastDay.Month; m != 1 {
		...
		if d := firstWeekLastDay.Day; d != 5 {
			t.Fatal(fmt.Sprintf("%d年第一周最后一天有误", year), d, "!=", 5)
			return
		}
		if w := firstWeekLastDay.Weekday; w != 7 {
			t.Fatal(fmt.Sprintf("%d年第一周最后一天星期几有误", year), w, "!=", 7)
		...
		if d := lastWeekFirstDay.Day; d != 29 {
			t.Fatal(fmt.Sprintf("%d年最后一周第一天有误", year), d, "!=", 29)
			return
		}
		if w := lastWeekFirstDay.Weekday; w != 1 {
			t.Fatal(fmt.Sprintf("%d年最后一周第一天星期几有误", year), w, "!=", 1)
		...
		if w := lastWeekLastDay.Weekday; w != 3 {
			t.Fatal(fmt.Sprintf("%d年最后一周最后一天星期几有误", year), w, "!=", 3)
			return
		}
		if w := lastWeekLastDay.YearWeeks; w != 53 {
			t.Fatal(fmt.Sprintf("%d年的年周有误", year), w, "!=", 53)
